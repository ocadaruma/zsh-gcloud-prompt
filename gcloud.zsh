...
function _is_gcloud_config_updated() {
    ...
    active_config_now="$(zstat +mtime $active_config 2>/dev/null)"
    #config_default_now="$(stat $mtime_fmt $config_default 2>/dev/null)"
    config_default_now="$(zstat +mtime $config_default 2>/dev/null)"
    #configurations_now="$(stat $mtime_fmt $configurations 2>/dev/null)"
    configurations_now="$(zstat +mtime $configurations 2>/dev/null)"
